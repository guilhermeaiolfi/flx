...
EmberLayoutManager.prototype = {
  ...
  createView: function(view, ctx) {
	...
	enter: function (ctx) {
		...
		var options = {
			view: view_class,
			outlet: current_route.options && current_route.options.outlet ? ctx.current_route.options.outlet : "main",
  ...
  render: function(view_name, options) {
		var view_to_render = options.view;
		var into = options.into;
    var parentView = this.lookupActiveView(into);
    var view = null;
    if (!parentView) {
       view = this.lookupActiveView(view_name);
       if (!view) {
        view = this.createView(view_to_render, options.routing);
